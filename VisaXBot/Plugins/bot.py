...
@bot.on(visa_cmd("kickme", outgoing=True))
async def leave(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("😪 **KThnxBye** See u all in visa!!")
        time.sleep(1)
        if "-" in str(e.chat_id):
...
@bot.on(visa_cmd(pattern=r"config"))
...
@bot.on(visa_cmd(pattern="schd ?(.*)", outgoing=True))
