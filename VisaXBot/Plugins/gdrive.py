...
async def save_response_content(response, destination):
    ...
    with open(destination, "wb") as f:
        for chunk in response.iter_content(CHUNK_SIZE):
            if chunk:  # filter out keep-visa new chunks
                f.write(chunk)
    return destination
...
CmdHelp("gdrive").add_command(
  "gdl", "gdrive link", "Downloads the file from gdirve to VisaXBot's local storage. Use .upload to upload it."
).add_info(
