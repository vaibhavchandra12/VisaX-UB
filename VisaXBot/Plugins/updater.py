...
from VisaXBot.helpers import runner
from . import *
...
async def update(event, repo, ups_rem, ac_br):
    ...
    args = [sys.executable, "-m", "VisaXBot"]
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        await event.delete()
        return await event.respond(f"🌚 Do `{hl}update build` to update your **∂єα∂ℓу кααℓ Bσт** !!")
...
CmdHelp("update").add_command(
  ...
  "update now", None, "Soft-Update Your ∂єα∂ℓу кααℓ Bσт. Basically if you restart dyno it will go back to previous deploy."
).add_command(
