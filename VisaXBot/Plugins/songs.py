...
@bot.on(visa_cmd(pattern="lyrics(?: |$)(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="lyrics(?: |$)(.*)", allow_sudo=True))
async def nope(kraken):
    visa = kraken.pattern_match.group(1)
    await eor(kraken, f"Searching lyrics for  `{visa}` ...")
    if not visa:
        if kraken.is_reply:
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(visa))}")
...
@bot.on(visa_cmd(pattern="song(?: |$)(.*)", outgoing=True))
...
@bot.on(visa_cmd(pattern="vsong(?: |$)(.*)", outgoing=True))
