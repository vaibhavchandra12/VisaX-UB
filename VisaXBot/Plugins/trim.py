...
@bot.on(deadly_cmd(pattern="tsave$"))
...
async def ff_mpeg_trim_cmd(event):
    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if reply_message:
            ...
            deadlyevent = await eor(event, "`Saving the file...`")
            try:
                c_time = time.time()
...
@bot.on(deadly_cmd(pattern="vtrim"))
...
async def ff_mpeg_trim_cmd(event):
    ...
    deadlyevent = await eor(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        try:
            ...
            return await eod(deadlyevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        ...
        await eod(deadlyevent, "RTFM")
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await eod(deadlyevent, f"`Completed Process in {ms} seconds`", 3)


@bot.on(deadly_cmd(pattern="atrim"))
...
@bot.on(deadly_cmd(pattern="tclear$"))
