...
@bot.on(deadly_cmd(pattern="scan ?(.*)"))
...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    deadlyevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(deadlyevent, "`Please unblock `@DrWebBot `and try again`")
            return
        if response.text.startswith("Forward"):
            await eod(deadlyevent,
...
@bot.on(deadly_cmd(pattern=r"decode$", outgoing=True))
...
@bot.on(deadly_cmd(pattern="barcode ?(.*)"))
@bot.on(sudo_cmd(pattern="barcode ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    deadlyevent = await eor(event, "...")
...
@bot.on(deadly_cmd(pattern=r"makeqr(?: |$)([\s\S]*)", outgoing=True))
...
async def _(event):
    ...
    start = datetime.datetime.now()
    input_str = event.pattern_match.group(1)
    deadly = await eor(event, "Processing...")
    ...
    if len(input_sgra) == 3:
        ...
        await deadly.edit(str(a))
    else:
        await eod(deadly, f"SYNTAX: {hl}calendar YYYY.MM.DD")
    end = datetime.datetime.now()
    (end - start).seconds


@bot.on(deadly_cmd(pattern="currency (.*)"))
...
@bot.on(deadly_cmd(pattern="currencies$"))
...
@bot.on(deadly_cmd(pattern="ifsc (.*)"))
...
@bot.on(deadly_cmd(pattern="color (.*)"))
...
@bot.on(deadly_cmd(pattern="xkcd ?(.*)"))
@bot.on(sudo_cmd(pattern="xkcd ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    deadlyevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await eod(deadlyevent, "xkcd n.{} not found!".format(xkcd_id))

@bot.on(deadly_cmd(pattern="dns (.*)", outgoing=True))
...
@bot.on(deadly_cmd(pattern="url (.*)", outgoing=True))
...
@bot.on(deadly_cmd(pattern="unshort (.*)", outgoing=True))
