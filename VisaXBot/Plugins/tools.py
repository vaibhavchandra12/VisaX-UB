...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    visaevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(visaevent, "`Please unblock `@DrWebBot `and try again`")
            return
...
@bot.on(visa_cmd(pattern=r"decode$", outgoing=True))
...
@bot.on(visa_cmd(pattern="barcode ?(.*)"))
@bot.on(sudo_cmd(pattern="barcode ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    visaevent = await eor(event, "...")
    ...
    try:
        ...
        await visaevent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await visaevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await visaevent.delete()


@bot.on(visa_cmd(pattern=r"makeqr(?: |$)([\s\S]*)", outgoing=True))
...
async def _(event):
    ...
    start = datetime.datetime.now()
    input_str = event.pattern_match.group(1)
    visa = await eor(event, "Processing...")
    ...
    if len(input_sgra) == 3:
        ...
        await visa.edit(str(a))
    else:
        await eod(visa, f"SYNTAX: {hl}calendar YYYY.MM.DD")
    end = datetime.datetime.now()
    (end - start).seconds


@bot.on(visa_cmd(pattern="currency (.*)"))
...
@bot.on(visa_cmd(pattern="currencies$"))
...
@bot.on(visa_cmd(pattern="xkcd ?(.*)"))
@bot.on(sudo_cmd(pattern="xkcd ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    visaevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await visaevent.edit(output_str, link_preview=True)
    else:
        await eod(visaevent, "xkcd n.{} not found!".format(xkcd_id))

@bot.on(visa_cmd(pattern="dns (.*)", outgoing=True))
...
@bot.on(visa_cmd(pattern="url (.*)", outgoing=True))
...
@bot.on(visa_cmd(pattern="unshort (.*)", outgoing=True))
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for VisaXBot", ".currency 10 usd inr"
).add_command(
